...
const btnElemDgUtama = `
    ...
    <div class="btn-group ms-2" role="group" aria-label="Button Print" id="btnMenuPrintDgUtama">
      ...
      <ul class="dropdown-menu dropdown-print" id="menuPrintDgUtama">
        <li>
          <a href="../all_report_detail/master_satuan_pdf.php" target="_blank" class="dropdown-item" id="btnPDFDetailDgUtama">
...
const satuan_id = document.getElementById('satuan_id');
const satuan_name = document.getElementById('satuan_name');
...
const addModalUtama = async (e) => {
  modalUtamaTitle.innerHTML = 'Tambah Satuan';
  saveModalUtama = async (e) => {
    ...
    if (saveStateModalUtama === false) {
      ...
      try {
        ...
        const getResponse = await sendViaFetchForm('route.php?act=addSatuan', sendData);
...
const editModalUtama = async (e) => {
  ...
  modalUtamaTitle.innerHTML = 'Ubah Satuan';
  satuan_id.value = data.id_satuan;
  satuan_name.value = data.name_satuan;
  saveModalUtama = async (e) => {
    modalUtamaBtnSave.disabled = true;
    if (saveStateModalUtama === false) {
      saveStateModalUtama = true;
      try {
        const sendData = await getDataUtama();
        sendData.append('satuan_id', data.id_satuan);
        const getResponse = await sendViaFetchForm('route.php?act=editSatuan', sendData);
        alertComponent.sendAnAlertOnTry(getResponse, closeModalUtama);
      } catch (error) {
        alertComponent.sendAnAlertOnCatch(error);
      } finally {
        ...
        confirmComponent.btnConfirm.addEventListener('click', async () => {
          if (saveStateModalUtama === false) {
            ...
            try {
              ...
              sendData.append('satuan_id', data.id_satuan);
              const getResponse = await sendViaFetchForm('route.php?act=deleteSatuan', sendData);
