...
const btnElemDgUtama = `
    ...
    <div class="btn-group ms-2" role="group" aria-label="Button Print" id="btnMenuPrintDgUtama">
      ...
      <ul class="dropdown-menu dropdown-print" id="menuPrintDgUtama">
        <li>
          <a href="../all_report_detail/master_formula_pdf.php" target="_blank" class="dropdown-item" id="btnPDFDetailDgUtama">
...
const formula_id = document.getElementById('formula_id');
const formula_name = document.getElementById('formula_name');
const formula_rumus = document.getElementById('formula_rumus');
...
const addModalUtama = async (e) => {
  modalUtamaTitle.innerHTML = 'Tambah Formula';
  saveModalUtama = async (e) => {
    ...
    if (saveStateModalUtama === false) {
      ...
      try {
        ...
        const getResponse = await sendViaFetchForm('route.php?act=addFormula', sendData);
...
const editModalUtama = async (e) => {
  ...
  modalUtamaTitle.innerHTML = 'Ubah Formula';
  formula_id.value = data.id_formula;
  formula_name.value = data.name_formula;
  formula_rumus.value = data.rumus_formula;
  saveModalUtama = async (e) => {
    modalUtamaBtnSave.disabled = true;
    if (saveStateModalUtama === false) {
      saveStateModalUtama = true;
      try {
        const sendData = await getDataUtama();
        sendData.append('formula_id', data.id_formula);
        const getResponse = await sendViaFetchForm('route.php?act=editFormula', sendData);
        alertComponent.sendAnAlertOnTry(getResponse, closeModalUtama);
      } catch (error) {
        alertComponent.sendAnAlertOnCatch(error);
      } finally {
        ...
        confirmComponent.btnConfirm.addEventListener('click', async () => {
          if (saveStateModalUtama === false) {
            ...
            try {
              ...
              sendData.append('formula_id', data.id_formula);
              const getResponse = await sendViaFetchForm('route.php?act=deleteFormula', sendData);
