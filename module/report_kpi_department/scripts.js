    ...
    if (jsonData.length > 0) {
      ...
      for (const objectData of jsonData) {
        ...
        let elemTdLaporan = `
          ...
          <td class="align-middle">${objectData.define_kpidept}</td>
          ...
          <td class="align-middle text-end">${!IsEmpty(objectData.target_kpidept, true) ? number_format_big(objectData.target_kpidept, 2, '.', ',') : ''}</td>
        `;
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  $(dgUtamaCompanyInput).on('select2:select', clearingOnSelectCompany);
    ...
    ajax: {
      ...
      data: function (params) {
        return {
          ...
          page: params.page || 1,
          year: dgUtamaYearInput.value,
          company: dgUtamaCompanyInput.value
        };
      },
      processResults: function (data) {
        return {
  ...
  dgUtamaYearBtn.addEventListener('click', async (e) => {
    ...
    h1Page.innerHTML = `LAPORAN KPI DEPARTMENT (${yearProgress})<br>${nameCompany.toUpperCase()} - ${nameDepartment.toUpperCase()}`;
