...
const modalChangeUserCompanyKPI = document.getElementById('modalChangeUserCompanyKPI');
const modalChangeUserDepartmentKPI = document.getElementById('modalChangeUserDepartmentKPI');
...
const insertingTdChangeUser = async (data, funInsert, addDataChangeUser) => {
  ...
  const elemTd = `
    ...
    <td>
      <span style="padding-left: ${(data.index_kpidept.split('.').length - 2) * 12}px;">(${data.index_kpidept}) ${data.name_kpidept}</span>
    </td>
  ...
  for (const objectData of jsonData) {
    await insertingTdChangeUser(objectData,
      async (element, index) => {
        ...
        id_changeuser.value = objectData.id_kpidept;
    ...
    await insertingTdEditor(objectData,
      async (element, index) => {
        ...
        if (checkBooleanFromServer(objectData.terbit_kpidept)) {
          disableInput();
          name_kpidept.disabled = true;
          define_kpidept.disabled = true;
          control_cek_kpidept.disabled = true;
...
const funViewKPIYear = async () => {
  ...
  .then(jsonData => {
    ...
    if (jsonData.response === 'error') {
      ...
      if (jsonData.filter((element) => element.id_kpidept).length > 0) {
...
const resetModalChangeUser = async () => {
  ...
  modalChangeUserCompanyKPI.innerText = null;
  modalChangeUserDepartmentKPI.innerText = null;
...
const defaultOpenModalChangeUser = async () => {
  ...
  modalChangeUserCompanyKPI.innerText = nameCompanyProgress;
  modalChangeUserDepartmentKPI.innerText = nameDepartmentProgress;
}
