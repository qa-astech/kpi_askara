...
const modalChangeUserTitle = document.getElementById("modalChangeUserTitle");
const modalChangeUserTahunKPI = document.getElementById(
  "modalChangeUserTahunKPI"
);
...
const diseventAlert = () => {
  alertComponent.alertElem.removeEventListener(
    "hidden.bs.modal",
    closeModalEditor
  );
  alertComponent.alertElem.removeEventListener(
...
const getKpiNow = async () => {
  return new Promise(async (resolve, reject) => {
    ...
    const getResult = await sendViaFetchForm(
      "route.php?act=getKpiBisnisUnit",
      sendData
    );
...
const insertingTdChangeUser = async (data, funInsert, addDataChangeUser) => {
  ...
  const elemTd = `
    ...
    <td>
      <span style="padding-left: ${(data.index_kpibunit.split('.').length - 2) * 12}px;">(${data.index_kpibunit}) ${data.name_kpibunit}</span>
    </td>
...
const insertingTdEditor = async (
  ...
  elemTr.dataset.indexParent = !IsEmpty(indexParent)
    ? indexParent
    : data.index_sobject;
  ...
  let monthElem = "";
  let arrayMonth =
    IsEmpty(data.month_kpibunit) || IsEmpty(data.month_kpibunit.slice(1, -1))
      ? null
      : data.month_kpibunit.slice(1, -1).split(",");
  monthAcroShort.forEach((month) => {
    const foundData = !IsEmpty(arrayMonth)
      ? arrayMonth.find((element) => element === month)
      : null;
    monthElem += `<div class="col-2">
      <input type="checkbox" class="btn-check btn-check-month" id="month_kpibunit_${month}_${indexRowEditor}" name="month_kpibunit[${indexRowEditor}][${month}]" autocomplete="off" value="true" ${
      !IsEmpty(foundData) ? "checked" : ""
    }>
      <label class="btn btn-outline-warning text-center d-block fw-bold" for="month_kpibunit_${month}_${indexRowEditor}">${
  ...
  const elemTd = `
    ...
    <td class="text-end">${
      !IsEmpty(data.baseline_1, true)
        ? number_format_big(data.baseline_1, 2, ".", ",")
        : "-"
    }</td>
    <td class="text-end">${
      isSupport
        ? "-"
        : !IsEmpty(data.target_kpicorp, true)
        ? number_format_big(data.target_kpicorp, 2, ".", ",")
        : "-"
    }</td>
  ...
  elemTr.addEventListener("click", (e) => {
    if (
      ...
      elemTr.classList.contains("selected")
        ? elemTr.classList.remove("selected")
        : elemTr.classList.add("selected");
  ...
  for (const objectData of jsonData) {
    await insertingTdChangeUser(objectData,
      async (element, index) => {
        ...
        id_changeuser.value = objectData.id_kpibunit;
...
const insertTdEditor = async (jsonData) => {
  for (const objectData of jsonData) {
    await insertingTdEditor(
      objectData,
      async (element, index) => {
        dgEditorTbody.append(element);
      },
      async (element, index) => {
        const index_kpibunit = document.getElementById(
          "index_kpibunit_" + index
        );
        const name_kpibunit = document.getElementById("name_kpibunit_" + index);
        const define_kpibunit = document.getElementById(
          "define_kpibunit_" + index
        );
        const control_cek_kpibunit = document.getElementById(
          "control_cek_kpibunit_" + index
        );
        const target_kpibunit = document.getElementById(
          "target_kpibunit_" + index
        );
        ...
        const target_kpicorp = document.getElementById(
          "target_kpicorp_" + index
        );
        ...
        target_kpicorp.value = !IsEmpty(objectData.target_kpicorp, true)
          ? number_format_big(objectData.target_kpicorp, 2, ".", ",")
          : null;
        ...
        target_kpibunit.value = !IsEmpty(objectData.target_kpibunit, true)
          ? number_format_big(objectData.target_kpibunit, 2, ".", ",")
          : null;
        ...
        if (!IsEmpty(objectData.id_satuan)) {
          const optionSatuan = new Option(
            objectData.name_satuan,
            objectData.id_satuan,
            true,
            true
          );
          const dataSatuan = {};
        ...
        if (!IsEmpty(objectData.id_formula)) {
          const optionFormula = new Option(
            objectData.name_formula,
            objectData.id_formula,
            true,
            true
          );
          const dataFormula = {};
          ...
          $("#formula_kpibunit_" + index)
            .append(optionFormula)
            .trigger("change");
        ...
        // Set value data availability
        if (!IsEmpty(objectData.data_avail_id_department)) {
          const optionDataAvailability = new Option(
            objectData.data_avail_name_department,
            objectData.data_avail_id_department,
            true,
            true
          );
          const dataDataAvailability = {};
          dataDataAvailability.id = objectData.data_avail_id_department;
          dataDataAvailability.text = objectData.data_avail_name_department;
          $("#data_avail_kpibunit_" + index)
            .append(optionDataAvailability)
            .trigger("change");
        ...
        // Set value user PIC
        if (!IsEmpty(objectData.data_avail_id_usersetup)) {
          const optionUsers = new Option(
            objectData.data_avail_fullname_users,
            objectData.data_avail_id_usersetup,
            true,
            true
          );
          const dataUsers = {};
        ...
        if (!IsEmpty(objectData.polaritas_kpibunit)) {
          $("#polaritas_kpibunit_" + index)
            .val(objectData.polaritas_kpibunit)
            .trigger("change");
        }
          ...
          elemCascade.addEventListener("change", (e) => {
            if (e.target.checked) {
              if (e.target.value === "full-round") {
                target_kpibunit.value = !IsEmpty(
                  objectData.target_kpicorp,
                  true
                )
                  ? number_format_big(objectData.target_kpicorp, 2, ".", ",")
                  : null;
        ...
        if (objectData.status_kpi === "kpi_bisnis_unit_corps") {
          target_kpibunit.disabled =
            objectData.cascade_kpibunit === "full-round";
          disableInput();
        } else {
        ...
        if (checkBooleanFromServer(objectData.terbit_kpibunit)) {
          ...
          monthAcroShort.forEach((month) => {
            const elementMonth = document.getElementById(
              `month_kpibunit_${month}_${index}`
            );
      ...
      objectData.cascade_kpibunit === "triangle"
    );
  }
};
  ...
  await getKpiNow()
    .then((jsonData) => {
      ...
      if (jsonData.response === "error") {
        ...
        modalChangeUserTitle.innerText = `KPI ${companyInput[0].text.toUpperCase()} (Ubah user PIC)`;
        ...
        for (const objectData of jsonData) {
          ...
          let monthElem = "";
          let arrayMonth =
            IsEmpty(objectData.month_kpibunit) ||
            IsEmpty(objectData.month_kpibunit.slice(1, -1))
              ? null
              : objectData.month_kpibunit.slice(1, -1).split(",");
          monthAcroShort.forEach((month) => {
            const foundData = !IsEmpty(arrayMonth)
              ? arrayMonth.find((element) => element === month)
              : null;
            monthElem += `<div class="col-2">
            <input type="radio" class="btn-check btn-check-month" autocomplete="off" value="true" ${
              !IsEmpty(foundData) ? "checked" : "disabled"
            }>
            <label class="btn btn-outline-warning text-center w-100 fw-bold">${
              month.charAt(0).toUpperCase() + month.slice(1)
            }</label>
          </div>`;
          });
          const elemTd = `
          <td class="${IsEmpty(objectData.id_kpibunit) ? "text-danger" : ""} text-center">${
            objectData.id_kpibunit
              ? objectData.id_kpibunit
              : "(Silahkan atur terlebih dahulu)"
          }</td>
          ...
          <td class="text-end">${
            !IsEmpty(objectData.baseline_1, true)
              ? number_format_big(objectData.baseline_1, 2, ".", ",")
              : ""
          }</td>
          <td class="text-end">${
            !IsEmpty(objectData.target_kpicorp, true)
              ? number_format_big(objectData.target_kpicorp, 2, ".", ",")
              : ""
          }</td>
          <td class="text-end">${
            !IsEmpty(objectData.target_kpibunit, true)
              ? number_format_big(objectData.target_kpibunit, 2, ".", ",")
              : ""
          }</td>
          ...
          <td class="text-center ${
            checkBooleanFromServer(objectData.terbit_kpibunit)
              ? "text-success"
              : "text-danger"
          } fw-bold">${
            checkBooleanFromServer(objectData.terbit_kpibunit)
        ...
        if (lateDate) {
          const monthLast = monthAcro.filter(
            (arrData) =>
              parseInt(arrData.number) === parseInt(lateDate.getMonth()) + 1
          )[0];
          dgUtamaUserEntry.innerText =
            lateDate instanceof Date && !isNaN(lateDate.valueOf())
              ? lateDate.getDate() +
                " " +
                monthLast.full +
                " " +
                lateDate.getFullYear() +
                " " +
                lateDate.getHours() +
                ":" +
                lateDate.getMinutes() +
        ...
        funBtnPublishDgUtama = async (e) => {
          confirmComponent.setupconfirm(
            "Terbit KPI",
            "bg-primary",
            "text-white",
            "Anda yakin ingin terbitkan KPI ini sekarang?"
          );
          ...
          if (!IsEmpty(yearProgress) || !IsEmpty(companyProgress)) {
            confirmComponent.btnConfirm.addEventListener("click", async () => {
              try {
                ...
                sendData.append("year_kpi", yearProgress);
                sendData.append("company_kpi", companyProgress);
                const getResponse = await sendViaFetchForm(
                  "route.php?act=publishKpiBisnisUnit",
                  sendData
                );
            ...
            confirmComponent.btnCancel.addEventListener(
              "click",
              closeAlertComp
            );
        ...
        funBtnEditDgUtama = async (e) => {
          ...
          funModalEditorBtnSave = async () => {
            ...
            if (saveStateformEditor === false) {
              ...
              try {
                ...
                const getResponse = await sendViaFetchForm(
                  "route.php?act=editKpiBisnisUnit",
                  sendData
                );
          ...
          modalEditor._element.addEventListener(
            "shown.bs.modal",
            openModalEditor
          );
        ...
        if (jsonData.filter((element) => element.id_kpibunit).length > 0) {
...
const resetModalChangeUser = async () => {
  ...
  modalChangeUserTitle.innerText = null;
...
const addIndexingKPI = async (elemTr) => {
  ...
  if (trElem.length > 0) {
    for (const elemCheck of trElem) {
      ...
      if (
        indexParent === indexParentCheck &&
        jsonData.index_sobject === jsonDataCheck.index_sobject &&
        jsonData.alias_perspective === jsonDataCheck.alias_perspective
      ) {
        indexKPI++;
      }
    }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  modalAddFresh._element.addEventListener("show.bs.modal", () => {
    if (!modalAddFreshBackdrop.classList.contains("show"))
      modalAddFreshBackdrop.classList.add("show");
  ...
  modalAddFresh._element.addEventListener("hide.bs.modal", () => {
    ...
    if (!modalAddFreshBackdrop.classList.contains("d-none"))
      modalAddFreshBackdrop.classList.add("d-none");
  });
  ...
  btnAddChildDgEditor.addEventListener("click", async (e) => {
    const trRow = document.querySelector(
      "table#dgEditor > tbody > tr.selected"
    );
    if (!IsEmpty(trRow)) {
      ...
      const indexInputForParent = trRow.querySelector(
        "input.index_kpibunit"
      ).value;
      const beginIndexAddElem =
        jsonData.alias_perspective + indexInputForParent + ".";
      const elementSame = [];
      ...
      await insertingTdEditor(
        ...
        async (element, index) => {
          ...
          if (trElem.length > 0) {
            for (const elemCheck of trElem) {
              const indexInputCheck = elemCheck.querySelector(
                "input.index_kpibunit"
              ).value;
              const jsonDataCheck = JSON.parse(elemCheck.dataset.rowJson);
              const trueIndex =
                jsonDataCheck.alias_perspective + indexInputCheck;
  ...
  btnDeleteDgEditor.addEventListener("click", (e) => {
    ...
    if (!IsEmpty(trRowSelector)) {
      ...
      if (jsonData.terbit_kpibunit !== "t") {
        ...
        arr1Second.pop();
        const findAlter1 =
          jsonData.alias_perspective + arr1Second.join(".") + ".";
        const dotCount = arr1.length - 1;
        confirmComponent.setupconfirm(
          "Hapus Data",
          "bg-danger",
          "text-white",
          "Apakah anda yakin ingin menghapus data tersebut? aksi ini akan menghapus turunannya sekaligus!"
        );

        const funRemoveElem = async () => {
          const elemAll = document.querySelectorAll(
            `table#dgEditor > tbody tr`
          );
        ...
        const funRewriteIndex = async () => {
          const elemToBeRerite = document.querySelectorAll(
            `table#dgEditor > tbody tr`
          );
          elemToBeRerite.forEach((elemCheck) => {
            ...
            if (findAlter2.includes(findAlter1) && comparisonResult < 0) {
              ...
              elemCheck.querySelector("input.index_kpibunit").value =
                splitArr.join(".");
            }
          });
        };
  ...
  modalAddFreshBtnSave.addEventListener("click", async (e) => {
    ...
    const beginIndexAddElem =
      jsonDataSelect[0].alias_perspective +
      jsonDataSelect[0].index_sobject +
      ".";
    const regexBeginIndexAddElem = new RegExp(
      beginIndexAddElem.replace(".", "\\.")
    );
      ...
      async (element, index) => {
        ...
        if (trElem.length > 0) {
          for (const elemCheck of trElem) {
            ...
            if (
              jsonDataCheck.alias_perspective === jsonData.alias_perspective &&
              parseInt(jsonDataCheck.index_sobject) >
                parseInt(jsonData.index_sobject)
            ) {
