...
const btnElemDgUtama = `
    ...
    <div class="btn-group ms-2" role="group" aria-label="Button Print" id="btnMenuPrintDgUtama">
      ...
      <ul class="dropdown-menu dropdown-print" id="menuPrintDgUtama">
        <li>
          <a href="../all_report_detail/strategic_objective_pdf.php" target="_blank" class="dropdown-item" id="btnPDFDetailDgUtama">
    ...
    function createTable() {
      return new Promise((resolve, reject) => {
        const table = $('#dgUtama').DataTable({
          ...
          `,
          ajax: {
            url: 'route.php?act=getStrategicObjective',
            type: 'POST'
          },
          columns: [
            { data: 'id_sobject' },
            { data: 'text_perspective' },
            { data: 'index_sobject' },
            { data: 'text_sobject' },
            {
              data: 'user_entry',
...
const sobject_id = document.getElementById('sobject_id');
const sobject_name = document.getElementById('sobject_name');
const sobject_index = document.getElementById('sobject_index');

let stateIndexSobject = null;
...
$('#perspective_id').select2({
  ...
  allowClear: true,
  ajax: {
    url: "../perspective/route.php?act=jsonPerspective",
    dataType: 'json',
    method: 'POST',
    delay: 250,
...
const addModalUtama = async (e) => {
  modalUtamaTitle.innerHTML = 'Tambah Strategi Objektif';
  sobject_index.disabled = true;
  saveModalUtama = async (e) => {
    ...
    if (saveStateModalUtama === false) {
      ...
      try {
        ...
        const getResponse = await sendViaFetchForm('route.php?act=addStrategicObjective', sendData);
...
const editModalUtama = async (e) => {
  ...
  modalUtamaTitle.innerHTML = 'Ubah Strategi Objektif';
  sobject_index.disabled = false;
  sobject_id.value = data.id_sobject;
  sobject_name.value = data.name_sobject;
  sobject_index.value = data.index_sobject;
  stateIndexSobject = data.index_sobject;
  
  // Set value perspektif
  const optionPerspective = new Option(data.text_perspective, data.id_perspective, true, true);
  const dataPerspective = {};
  dataPerspective.id = data.id_perspective;
  dataPerspective.text = data.text_perspective;
  $('#perspective_id').append(optionPerspective).trigger('change');
  $('#perspective_id').trigger({
    type: 'select2:select',
    params: {
...
document.addEventListener("DOMContentLoaded", () => {
        ...
        confirmComponent.btnConfirm.addEventListener('click', async () => {
          if (saveStateModalUtama === false) {
            ...
            try {
              ...
              sendData.append('sobject_id', data.id_sobject);
              const getResponse = await sendViaFetchForm('route.php?act=deleteStrategicObjective', sendData);
  ...
  sobject_index.addEventListener('keyup', (e) => {
    ...
    if (IsEmpty(value)) {
      ...
      input.value = stateIndexSobject;
    } else {
      stateIndexSobject = input.value;
    }
  });

  modalUtama._element.addEventListener('hidden.bs.modal', (e) => {
    stateIndexSobject = null;
  });
