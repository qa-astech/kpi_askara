...
const btnElemDgUtama = `
    ...
    <div class="btn-group ms-2" role="group" aria-label="Button Print" id="btnMenuPrintDgUtama">
      ...
      <ul class="dropdown-menu dropdown-print" id="menuPrintDgUtama">
        <li>
          <a href="../all_report_detail/master_plant_pdf.php" target="_blank" class="dropdown-item" id="btnPDFDetailDgUtama">
...
const plant_id = document.getElementById('plant_id');
const plant_name = document.getElementById('plant_name');
const plant_alias = document.getElementById('plant_alias');
...
const addModalUtama = async (e) => {
  modalUtamaTitle.innerHTML = 'Tambah Plan';
  saveModalUtama = async (e) => {
    ...
    if (saveStateModalUtama === false) {
      ...
      try {
        ...
        const getResponse = await sendViaFetchForm('route.php?act=addPlant', sendData);
...
const editModalUtama = async (e) => {
  ...
  modalUtamaTitle.innerHTML = 'Ubah Plan';
  plant_id.value = data.id_plant;
  plant_name.value = data.name_plant;
  plant_alias.value = data.alias_plant;
  saveModalUtama = async (e) => {
    modalUtamaBtnSave.disabled = true;
    if (saveStateModalUtama === false) {
      saveStateModalUtama = true;
      try {
        const sendData = await getDataUtama();
        sendData.append('plant_id', data.id_plant);
        const getResponse = await sendViaFetchForm('route.php?act=editPlant', sendData);
        alertComponent.sendAnAlertOnTry(getResponse, closeModalUtama);
      } catch (error) {
        alertComponent.sendAnAlertOnCatch(error);
      } finally {
        ...
        confirmComponent.btnConfirm.addEventListener('click', async () => {
          if (saveStateModalUtama === false) {
            ...
            try {
              ...
              sendData.append('plant_id', data.id_plant);
              const getResponse = await sendViaFetchForm('route.php?act=deletePlant', sendData);
